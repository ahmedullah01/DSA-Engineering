...
void swap(){
    Node* c1 = List; // First node
    Node* c2 = List->next; // Second node
    while(c1 !=NULL && c2!=NULL){
int temp = c1->data; // Swap data of first and second node
c1->data = c2->data;
c2->data = temp;
c1 = c2->next;
if(c1 ==NULL){
    break; // If no more pairs, exit loop
} // Move c1 to next pair
else{
    c2 = c1->next; // Move c2 to next pair
}
    }
}
